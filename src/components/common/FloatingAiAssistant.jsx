    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      ...
      {isOpen && (
        <div className="mb-4 w-80 sm:w-96 bg-white rounded-3xl shadow-2xl border border-gray-200 overflow-hidden animate-fade-in flex flex-col h-[480px]">
          {/* Top Bar */}
          <div className="bg-[#18392B] text-white p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#C85A32] flex items-center justify-center text-white shadow-md">
          ...
          {/* Chat Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#FBF9F5]">
            {messages.map((msg, idx) => (
              ...
              >
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs shrink-0 ${
                  msg.sender === "user" ? "bg-[#C85A32] text-white" : "bg-[#18392B] text-white"
                }`}>
                  {msg.sender === "user" ? <User className="w-4 h-4" /> : <Sparkles className="w-3.5 h-3.5 text-[#D4A373]" />}
                </div>
                <div
          ...
          <div className="px-3 py-2 bg-white border-t border-gray-100 flex gap-1.5 overflow-x-auto text-[11px]">
            <button
              ...
              className="px-2.5 py-1 bg-gray-100 hover:bg-[#C85A32]/10 hover:text-[#C85A32] rounded-full whitespace-nowrap text-gray-600 transition-colors"
            >
              💍 Áo dài cưới
            </button>
            <button
          ...
          <form onSubmit={handleSend} className="p-3 bg-white border-t border-gray-100 flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 bg-[#FBF9F5] border border-gray-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:border-[#18392B]"
            />
            <button
              type="submit"
              className="p-2.5 bg-[#18392B] text-white rounded-xl hover:bg-[#18392B]/90 transition-colors cursor-pointer"
            >
      ...
      <button
        ...
        className="w-14 h-14 rounded-full bg-[#18392B] text-white shadow-xl shadow-[#18392B]/30 hover:shadow-2xl hover:scale-105 flex items-center justify-center transition-all cursor-pointer relative"
      ...
      >
        {isOpen ? (
          ...
          <>
            ...
            <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#C85A32] text-white text-[9px] font-bold rounded-full flex items-center justify-center border-2 border-white animate-bounce">
              AI
            </span>
          </>
        )}
