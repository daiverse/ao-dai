      ...
      <div className="fixed inset-y-0 right-0 max-w-xs w-full bg-white shadow-2xl flex flex-col justify-between p-6 animate-fade-in">
        <div>
          <div className="flex items-center justify-between pb-6 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full overflow-hidden border border-[#E8C55A]/40 flex items-center justify-center">
                <img src="/logo.jpg" alt="DaiVerse" className="w-full h-full object-cover" />
              </div>
              <span className="font-heading font-bold text-lg text-[#C8920A]">DaiVerse</span>
            </div>
            ...
            {navLinks.map((link) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl flex items-center justify-between text-sm font-medium transition-all ${
                  activeTab === link.id
                    ? "bg-[#FFDF00] text-[#2C1A00] font-semibold"
                    : "text-gray-700 hover:bg-gray-100"
                }`}
              >
                <span>{link.label}</span>
                <ChevronRight className={`w-4 h-4 ${activeTab === link.id ? "text-[#E8C55A]" : "text-gray-400"}`} />
              </button>
            ))}
          </nav>
        </div>
        ...
        <div className="pt-6 border-t border-gray-100">
          <button
            ...
            className="w-full py-3 bg-[#C8920A] text-white rounded-xl font-medium flex items-center justify-center gap-2 shadow-lg shadow-[#C8920A]/25"
          >
