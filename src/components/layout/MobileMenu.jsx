      ...
      <div className="fixed inset-y-0 right-0 max-w-xs w-full bg-white shadow-2xl flex flex-col justify-between p-6 animate-fade-in">
        <div>
          <div className="flex items-center justify-between pb-6 border-b border-gray-100">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full overflow-hidden border border-[#D4A373]/40 flex items-center justify-center">
                <img src="/logo.jpg" alt="DaiVerse" className="w-full h-full object-cover" />
              </div>
              <span className="font-heading font-bold text-lg text-[#18392B]">DaiVerse</span>
            </div>
          ...
          <nav className="mt-6 space-y-2">
            {navLinks.map((link) => (
              ...
              >
                ...
                <ChevronRight className={`w-4 h-4 ${activeTab === link.id ? "text-[#D4A373]" : "text-gray-400"}`} />
              </button>
            ))}
          </nav>
        </div>
        ...
        <div className="pt-6 border-t border-gray-100">
          <button
            ...
            className="w-full py-3 bg-[#C85A32] text-white rounded-xl font-medium flex items-center justify-center gap-2 shadow-lg shadow-[#C85A32]/25"
          >
