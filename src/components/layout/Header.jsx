    ...
    <header className="fixed top-2 sm:top-3 left-0 right-0 z-50 transition-all duration-300 px-3 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <div className={`relative flex items-center justify-between px-3 sm:px-5 lg:px-6 h-16 rounded-full transition-all duration-500 border ${
          ...
          >
            <div className="relative w-10 h-10 sm:w-11 sm:h-11 rounded-full overflow-hidden border border-[#EFB11D]/40 shadow-sm group-hover:scale-105 transition-transform bg-[#EFB11D] flex items-center justify-center">
              <img src="/logo.jpg" alt="DaiVerse" className="w-full h-full object-cover" />
            </div>
            <div className="ml-2.5 hidden sm:block">
              <span className="font-heading font-bold text-lg sm:text-xl text-[#EFB11D] block leading-none">DaiVerse</span>
            </div>
          </button>
            ...
            {navItems.map((item) => (
              <button
                ...
                className={`text-xs lg:text-[13px] xl:text-[14px] font-medium transition-all relative group whitespace-nowrap flex items-center gap-1 border-none bg-transparent outline-none p-0 appearance-none ${
                  activeTab === item.id 
                    ? "text-[#E43D12] font-semibold" 
                    : item.isExpress
                      ? "text-[#E43D12] font-semibold hover:text-[#EFB11D]"
                      : "text-gray-700 hover:text-[#E43D12]"
                }`}
              >
                <span>{item.label}</span>
                {item.isExpress && (
                  <span className="px-1 py-0.5 text-[9px] font-bold bg-[#E43D12] text-white rounded tracking-tight leading-none animate-pulse">
                    24h
                  </span>
                )}
                <span className={`absolute -bottom-1 left-0 h-[2px] bg-[#E43D12] transition-all duration-300 ${
          ...
          <div className="hidden lg:flex items-center gap-2 lg:gap-2.5 z-10 relative shrink-0 ml-1 lg:ml-2 xl:ml-4">
            ...
            {isAuthenticated ? (
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full border bg-white transition-all cursor-pointer shadow-sm hover:shadow-md ${
                    justLoggedIn
                      ? "border-[#EFB11D] ring-4 ring-[#EFB11D]/50 shadow-xl scale-105 animate-pulse"
                      : "border-gray-200 hover:border-[#E43D12]/40"
                  }`}
                >
                  <div className="w-7 h-7 rounded-full overflow-hidden bg-[#EFB11D] text-[#2C1A00] flex items-center justify-center text-xs font-bold shrink-0 shadow-inner">
                    {user.avatar ? (
                ...
                {isUserDropdownOpen && (
                  <div className="absolute right-0 mt-2.5 w-60 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 z-50 animate-fade-in">
                    <div className="px-3.5 py-3 border-b border-gray-100 bg-[#EBE9E1] rounded-xl mb-1">
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-full text-xs font-bold text-[#2C1A00] border border-[#EFB11D]/60 hover:bg-[#EFB11D] hover:text-[#2C1A00] transition-all cursor-pointer bg-transparent whitespace-nowrap"
              >
                Đăng Nhập
              </button>
            )}
            ...
            <button
              ...
              aria-label="Giỏ hàng"
              className="relative p-2 rounded-full text-gray-700 hover:text-[#E43D12] hover:bg-[#E43D12]/10 transition-all cursor-pointer border-none bg-transparent outline-none appearance-none"
            >
              <ShoppingBag className="w-5 h-5" />
              {totalItems > 0 && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-[#E43D12] text-white text-[10px] font-bold rounded-full flex items-center justify-center animate-pulse">
                  {totalItems}
                </span>
              )}
            </button>
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 lg:px-4 py-2 rounded-full font-medium transition-all text-xs xl:text-sm bg-[#EFB11D] text-[#2C1A00] shadow-md shadow-[#EFB11D]/40 hover:bg-[#EFB11D]/90 hover:shadow-lg cursor-pointer border-none outline-none appearance-none"
              >
                <Sparkles className="w-3.5 h-3.5 text-[#EFB11D] animate-pulse" />
              ...
              {isAiDropdownOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 z-50 animate-fade-in">
                  <div className="px-3 py-2 border-b border-gray-100">
                    <p className="text-[11px] font-semibold text-[#E43D12] uppercase tracking-wider">Bộ Công Cụ AI 2026</p>
                  </div>
                  {aiExperiences.map((exp) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left p-2.5 rounded-xl hover:bg-[#EBE9E1] transition-colors flex items-center gap-3 group cursor-pointer border-none bg-transparent outline-none appearance-none"
                      >
                        <div className="w-9 h-9 rounded-xl bg-[#EFB11D]/10 text-[#EFB11D] group-hover:bg-[#E43D12] group-hover:text-white transition-all flex items-center justify-center shrink-0">
                          <IconComponent className="w-4 h-4" />
                        </div>
                        <div>
                          <p className="text-sm font-semibold text-gray-900 group-hover:text-[#E43D12] transition-colors">
                            {exp.title}
                          </p>
          ...
          <div className="flex lg:hidden items-center gap-2 z-10 shrink-0">
            <button
              onClick={() => setIsCartOpen(true)}
              aria-label="Giỏ hàng"
              className="relative p-2 rounded-full text-gray-700 hover:text-[#E43D12]"
            >
              <ShoppingBag className="w-5 h-5" />
              {totalItems > 0 && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-[#E43D12] text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                  {totalItems}
                </span>
              )}
            </button>
            ...
            <button
              ...
              className="p-2 rounded-full text-[#EFB11D] bg-[#EFB11D]/10 hover:bg-[#EFB11D]/20 transition-colors cursor-pointer border-none outline-none"
            >
            ...
            <button
              ...
              className="p-2 text-gray-700 hover:text-[#E43D12] transition-colors cursor-pointer border-none bg-transparent outline-none"
              aria-label="Mở menu"
            >
