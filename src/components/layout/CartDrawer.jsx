    ...
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col justify-between animate-fade-in">
          {/* Header */}
          <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-[#FBF9F5]">
            <div className="flex items-center gap-2">
              <ShoppingBag className="w-5 h-5 text-[#18392B]" />
              <h2 className="font-heading text-xl font-bold text-[#18392B]">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {cart.length === 0 ? (
              <div className="text-center py-16 space-y-4">
                <div className="w-16 h-16 bg-[#FBF9F5] rounded-full flex items-center justify-center mx-auto text-gray-400">
                  <ShoppingBag className="w-8 h-8 text-[#C85A32]/60" />
                </div>
                <p className="text-gray-500 font-medium">Giỏ hàng của bạn đang trống</p>
                <button
                  onClick={() => setIsCartOpen(false)}
                  className="px-6 py-2.5 bg-[#18392B] text-white text-sm rounded-full font-medium hover:bg-[#18392B]/90 transition-all shadow-md"
                >
              ...
              cart.map((item, index) => (
                <div key={index} className="flex gap-4 p-3 bg-[#FBF9F5] rounded-2xl border border-gray-100 relative group">
                  <img
          ...
          {cart.length > 0 && (
            <div className="p-6 border-t border-gray-100 bg-[#FBF9F5] space-y-4">
              <div className="flex items-center justify-between text-base">
                <span className="text-gray-600 font-medium">Tổng tiền tạm tính:</span>
                <span className="font-heading font-bold text-xl text-[#18392B]">{formattedTotalPrice}</span>
              </div>
              ...
              <button
                ...
                className="w-full py-4 bg-[#18392B] text-white rounded-xl font-medium flex items-center justify-center gap-2 hover:bg-[#18392B]/90 transition-all shadow-xl shadow-[#18392B]/20 cursor-pointer"
              >
