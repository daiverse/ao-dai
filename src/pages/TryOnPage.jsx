...
export default function TryOnPage({ selectedProductFromState }) {
  ...
  return (
    <div className="pt-28 pb-20 bg-[#EBE9E1] min-h-screen text-gray-900">
      <div className="container-page">
        {/* Title Section */}
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#EFB11D]/10 text-[#EFB11D] text-xs font-bold uppercase tracking-wider mb-3 border border-[#EFB11D]/20">
          ...
          <h1 className="font-heading text-4xl sm:text-5xl font-bold text-gray-900 leading-tight">
            Thử Áo Dài Thực Tế <span className="text-[#EFB11D] italic">Chuẩn Tỉ Lệ Dáng</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-6 bg-white p-6 sm:p-8 rounded-3xl shadow-xl border border-gray-100 space-y-8">
            ...
            <div>
              <h3 className="font-heading font-bold text-lg text-gray-900 mb-4 flex items-center gap-2">
                <span className="w-7 h-7 rounded-full bg-[#EFB11D] text-white text-xs font-sans flex items-center justify-center">1</span>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-2xl border transition-all text-center cursor-pointer ${
                      selectedAvatar === idx && viewMode !== "user"
                        ? "border-[#EFB11D] bg-[#EFB11D]/5 ring-2 ring-[#EFB11D]/30"
              ...
              {/* Upload Box */}
              <label className="p-4 border-2 border-dashed border-gray-300 rounded-2xl text-center bg-[#EBE9E1] hover:border-[#EFB11D] transition-colors cursor-pointer block">
                <input type="file" accept="image/*" className="hidden" onChange={handleUserImageUpload} />
                {userUploadedImage ? (
                  <div className="flex items-center gap-3">
                    <img src={userUploadedImage} alt="Ảnh của bạn" className="w-14 h-14 object-cover rounded-xl border-2 border-[#EFB11D]" />
                    <div className="text-left">
                      <p className="text-xs font-bold text-[#EFB11D]">✓ Đã chọn ảnh của bạn</p>
            ...
            <div>
              <h3 className="font-heading font-bold text-lg text-gray-900 mb-4 flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <span className="w-7 h-7 rounded-full bg-[#EFB11D] text-white text-xs font-sans flex items-center justify-center">2</span>
                  Chọn Mẫu Áo Dài Để Thử
                </span>
                <span className="text-xs text-[#E43D12] font-semibold">{PRODUCTS.length} Mẫu BST Mộc Lan</span>
              </h3>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-2xl border text-left transition-all cursor-pointer ${
                      selectedProduct.id === prod.id
                        ? "border-[#E43D12] bg-[#E43D12]/5 ring-2 ring-[#E43D12]/30 shadow-md"
                        : "border-gray-200 hover:border-gray-300"
                    }`}
                  >
                    <div className="relative aspect-[3/4] rounded-xl overflow-hidden mb-2 bg-gray-100">
                      ...
                      {selectedProduct.id === prod.id && (
                        <div className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-[#E43D12] text-white flex items-center justify-center text-[10px] font-bold">
                          ✓
                        </div>
                      )}
                    </div>
                    <p className="font-semibold text-xs text-gray-900 line-clamp-1">{prod.name}</p>
                    <p className="text-xs text-[#E43D12] font-bold mt-0.5">{prod.formattedPrice}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-[#EBE9E1] border border-gray-200/80">
              <div>
                  ...
                  {selectedProduct.sizes?.map((sz, sIdx) => (
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-xl text-xs font-bold border transition-all cursor-pointer ${
                        selectedSize === sz
                          ? "border-[#E43D12] bg-[#E43D12] text-white shadow-sm"
            ...
            <button
              ...
              className="w-full py-4 bg-[#EFB11D] text-white font-bold rounded-2xl hover:bg-[#EFB11D]/90 shadow-xl shadow-[#EFB11D]/20 flex items-center justify-center gap-2 transition-all cursor-pointer text-sm disabled:opacity-60"
            >
              <Sparkles className={`w-5 h-5 text-[#EFB11D] ${isProcessing ? "animate-spin" : ""}`} />
          ...
          <div className="lg:col-span-6 bg-white p-6 sm:p-8 rounded-3xl shadow-xl border border-gray-100 sticky top-28 space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              {/* View Mode Buttons */}
              <div className="flex items-center gap-1 bg-[#EBE9E1] p-1 rounded-xl border border-gray-200">
                <button
                  onClick={() => setViewMode("model")}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${
                    viewMode === "model" ? "bg-[#EFB11D] text-white shadow-xs" : "text-gray-600 hover:text-gray-900"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${
                    viewMode === "detail" ? "bg-[#EFB11D] text-white shadow-xs" : "text-gray-600 hover:text-gray-900"
                  }`}
                >
                ...
                {userUploadedImage && (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer ${
                      viewMode === "user" ? "bg-[#EFB11D] text-white shadow-xs" : "text-gray-600 hover:text-gray-900"
                    }`}
                  >
            ...
            <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-gray-900 border border-gray-200 shadow-2xl flex items-center justify-center group">
              {isProcessing ? (
                <div className="flex flex-col items-center gap-4 text-center p-8 text-white">
                  <div className="w-16 h-16 rounded-full bg-[#EFB11D] border border-[#EFB11D]/40 flex items-center justify-center text-[#EFB11D]">
                  ...
                  <div>
                    ...
                    <div className="mt-4 w-48 bg-white/20 h-1.5 rounded-full overflow-hidden mx-auto">
                      <div className="bg-[#E43D12] h-full w-3/4 animate-pulse rounded-full"></div>
                    </div>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-6 text-white space-y-1">
                    <p className="text-xs uppercase tracking-widest text-[#EFB11D] font-bold">
            ...
            <div className="space-y-4 pt-2">
              <div className="p-4 bg-[#EBE9E1] rounded-2xl border border-gray-200 flex items-center justify-between">
                <div>
                ...
                <div className="text-right">
                  <span className="font-bold text-lg font-heading text-[#EFB11D] block">{selectedProduct.formattedPrice}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  ...
                  className="py-4 bg-[#E43D12] text-white font-bold rounded-2xl hover:bg-[#E43D12]/90 shadow-xl shadow-[#E43D12]/25 flex items-center justify-center gap-2 transition-all cursor-pointer text-sm"
                >
                ...
                <button
                  ...
                  className="py-4 bg-[#EFB11D] text-white font-bold rounded-2xl hover:bg-[#EFB11D]/90 shadow-xl shadow-[#EFB11D]/25 flex items-center justify-center gap-2 transition-all cursor-pointer text-sm"
                >
